...
def get_processor_path():
    if getattr(sys, 'frozen', False):  # Проверка, запущено ли как exe
        return os.path.join(sys._MEIPASS, "video_processor.exe")
    return "video_processor.exe"  # Обычный путь при запуске скрипта
...
def start_processing(input_path, output_path, progress_bar, start_button):
    processor_path = get_processor_path()
...
def main():
    root = tk.Tk()
    root.title("SafeView")
    root.geometry("500x330")  # Установленный размер окна
    ...
    start_button = create_glow_button("Начать обработку", None)
