...
def handle_video_or_document(message):
    try:
        ...
        bot.reply_to(message, "Видео получено, начинается обработка...")
        ...
        # Обработка видео
        process_video(input_path, output_path, "video_processor.exe")
...
def echo_all(message):
    bot.reply_to(message, "Это не видеофайл. Отправь видеофайл (MP4, AVI, MOV, MKV, FLV, WMV) в виде видео или документа.")
